 ...
 */
class GlobalKomentarFragment : Fragment(), MainView {

    private lateinit var presenter: MainPresenter
    private var quotes: MutableList<Quote> = mutableListOf()
    private lateinit var adapter: GlobalKomentarAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = GlobalKomentarAdapter(quotes, activity!!)
        ...
        fab.setOnClickListener { view ->
            val intent = Intent(activity!!, KomentarDetailActivity::class.java)
            intent.putExtra(KomentarDetailActivity.REQUEST_CODE, addQuoteCode);
            startActivityForResult(intent, addQuoteCode)
        }
        adapter.setOnItemClickCallback(object : GlobalKomentarAdapter.OnItemClickCallback{
            override fun onItemClicked(data: Quote) {
                val intent = Intent(activity!!, KomentarDetailActivity::class.java)
                intent.putExtra(KomentarDetailActivity.REQUEST_CODE, editQuoteCode);
                intent.putExtra(KomentarDetailActivity.EXTRA_DATA, data)
